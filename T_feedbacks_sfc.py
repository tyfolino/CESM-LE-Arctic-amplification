...
# The following is for the surface fluxes
kern_names = ['FLNS','FLNSC']
...
for e in range(1,end,1):
    ...
    for k in range(len(kern_names)):
        ...
        tkern = np.where(np.isfinite(dT),tkern,np.nan)
        tskern = tskerns[k]
        
        # sign convention: into the surface is negative to correspond to OHU
        atm = (np.nansum(tkern * dp * dT,axis=1))  * -1
        sfc = (tskern * dTS) * -1
        
        # make our output into a data array to save out as netcdf file
        da = xr.DataArray(data=atm,dims=['time','lat','lon'],coords=dict(
        lon=(["lon"], lon.data),
        lat=(["lat"], lat.data),
        time=time.data,
            ),
        ...
        rad_pert_Ta.append(da)
        
        # make our output into a data array to save out as netcdf file
        da = xr.DataArray(data=sfc,dims=['time','lat','lon'],coords=dict(
        lon=(["lon"], lon.data),
        lat=(["lat"], lat.data),
        time=time.data,
            ),
    ...
    pathout = '/dx02/janoski/cesm/vert_int_feedbacks/b40.1850.cam5-lens.'+str(
        f"{m:02d}")+'.'+str(f"{e:02d}")+'.h1_Ta_tropo_sfc.nc'
    
    out.to_netcdf(pathout)
    
    out = xr.merge(rad_pert_Ts)
    out['lat'].attrs = lat.attrs
    out['lon'].attrs = lon.attrs
    out['time'].attrs = time.attrs

    pathout = '/dx02/janoski/cesm/vert_int_feedbacks/b40.1850.cam5-lens.'+str(
        f"{m:02d}")+'.'+str(f"{e:02d}")+'.h1_Ts_tropo_sfc.nc'
...
# # Purpose: this script will calculate the difference in air temperature, surface skin temperature, then
# # use the CAM5 kernels to produce radiative perturbations from changes in atmospheric and surface
# # temperature at the surface.

# # Kernel work flow: Make daily kernels in kernel_time_interp.ipynb -> vertically
# # regrid in save_out_kernels.ipynb
...
#     # for ctrl/exp, regrid T (air temp) to p levels
#     # pressure levels below surface become nan
#     ctrl = ngl.vinth2p(ctrl.T,ctrl.hyam,ctrl.hybm,plevs,ctrl.PS,1,ctrl.P0/100,1,kxtrp=False)
#     ctrl[(ctrl==1e30) | (np.expand_dims(plevs,axis=[0,2,3]) < np.expand_dims(trop_c.TROP_P/100,axis=1))] = np.nan
#     exp = ngl.vinth2p(exp.T,exp.hyam,exp.hybm,plevs,exp.PS,1,exp.P0/100,1,kxtrp=False)
